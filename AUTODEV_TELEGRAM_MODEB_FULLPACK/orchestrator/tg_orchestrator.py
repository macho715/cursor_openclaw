...
- 승인: Telegram `/approve <task_id>` 또는 `/approve`(마지막 PASS 작업) → **승인 플래그 생성 + (선택) PR 브랜치 생성(Checkout/Push/Commit은 금지)**
...
class Cfg:
    ...
    # PR branch (approve 이후 자동 생성, push/commit/apply는 금지)
    create_pr_branch_on_approve: bool
    pr_branch_prefix: str
    pr_branch_from: str
    deny_if_branch_exists: bool
...
def git_rev_parse(repo: Path, ref: str) -> Tuple[bool, str]:
    code, out = run_cmd(["git", "rev-parse", ref], cwd=repo, timeout=60)
    sha = out.strip().splitlines()[-1] if out.strip() else ""
    return (code == 0 and bool(sha)), sha


def git_check_branch_name(repo: Path, branch: str) -> bool:
    code, _ = run_cmd(["git", "check-ref-format", "--branch", branch], cwd=repo, timeout=60)
    return code == 0


def git_branch_exists(repo: Path, branch: str) -> bool:
    # refs/heads/<branch> 존재 여부
    code, _ = run_cmd(["git", "show-ref", "--verify", "--quiet", f"refs/heads/{branch}"], cwd=repo, timeout=60)
    return code == 0


def git_create_branch(repo: Path, branch: str, startpoint: str) -> Tuple[bool, str]:
    # checkout 없이 브랜치 포인터만 생성
    code, out = run_cmd(["git", "branch", branch, startpoint], cwd=repo, timeout=60)
    return (code == 0), out.strip()
...
def approve_task(task_id: str, chat_id: int, cfg: Cfg) -> Tuple[bool, str, Optional[Path]]:
    ...
    # (옵션) approve 이후 PR 브랜치 포인터 자동 생성 (checkout/apply/commit/push는 금지)
    pr_branch = ""
    base_sha = ""
    if cfg.create_pr_branch_on_approve:
        pr_branch = f"{cfg.pr_branch_prefix}{task_id}"
        if not git_check_branch_name(cfg.repo_root, pr_branch):
            return False, f"INVALID_BRANCH_NAME:{pr_branch}", None

        ok_sha, sha = git_rev_parse(cfg.repo_root, "HEAD")
        if not ok_sha:
            return False, "GIT_REV_PARSE_HEAD_FAIL", None
        base_sha = sha

        if git_branch_exists(cfg.repo_root, pr_branch):
            if cfg.deny_if_branch_exists:
                return False, f"BRANCH_ALREADY_EXISTS:{pr_branch}", None
        else:
            ok_br, out_br = git_create_branch(cfg.repo_root, pr_branch, base_sha if cfg.pr_branch_from == "HEAD" else cfg.pr_branch_from)
            if not ok_br:
                return False, f"BRANCH_CREATE_FAIL:{out_br[:800]}", None
    ...
    approved = {
        ...
        "pr_branch": pr_branch or None,
        "pr_base_sha": base_sha or None,
        "pr_branch_created": bool(pr_branch),
    }
...
def main() -> int:
    ...
    cfg = Cfg(
        ...
        # git/pr branch settings
        create_pr_branch_on_approve=bool(cfg_raw.get("git", {}).get("create_pr_branch_on_approve", False)),
        pr_branch_prefix=str(cfg_raw.get("git", {}).get("pr_branch_prefix", "autodev/")),
        pr_branch_from=str(cfg_raw.get("git", {}).get("pr_branch_from", "HEAD")),
        deny_if_branch_exists=bool(cfg_raw.get("git", {}).get("deny_if_branch_exists", False)),
    ...
    while True:
        ...
        for item in results:
            ...
            if text.startswith(cfg.approve_cmd):
                ...
                # 승인 파일을 읽어 pr_branch/patch_path 포함해서 안내
                ap = {}
                try:
                    ap = json.loads(approved_path.read_text(encoding="utf-8"))
                except Exception:
                    ap = {}
                pr_branch = ap.get("pr_branch") or f"{cfg.pr_branch_prefix}{task_id}"
                patch_path = ap.get("patch_path") or str(cfg.patch_dir / f"{task_id}.patch")
                msg_ok = cfg.tmpl_pass_approved.format(task_id=task_id, approved_path=str(approved_path), pr_branch=pr_branch, patch_path=patch_path)
                send_tg(cfg.tg_token, chat_id, msg_ok[: cfg.reply_max_chars])
                write_audit_line(
                    cfg.q_audit / "events.ndjson",
                    {"ts": _utc_iso(), "actor": "orchestrator", "action": "APPROVED_FLAG_CREATED", "task_id": task_id, "approved_path": str(approved_path), "pr_branch": pr_branch},
                )
                last_reply_at = time.time()
                continue
