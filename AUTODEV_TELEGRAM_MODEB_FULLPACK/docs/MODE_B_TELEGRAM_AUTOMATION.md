...
2) Telegram: `/approve <task_id>` (또는 `/approve` = 마지막 PASS 작업)
   - 오케스트레이터는 **승인 플래그 생성 + PR 브랜치 포인터 생성(Checkout/Apply/Commit/Push는 금지)**
     - 승인: `.autodev_queue/approved/<task_id>.approved.json`
     - PR 브랜치(기본): `autodev/<task_id>` (git branch만 생성)
3) Cursor에서만 적용:
```powershell
...
### PR 브랜치 정책(기본)
- `/approve` 시: `git branch autodev/<task_id> HEAD` (checkout 없음)
- Cursor 적용 스크립트 실행 시: 해당 브랜치로 checkout 후 patch apply
- Push는 수동: `git push -u origin autodev/<task_id>`
